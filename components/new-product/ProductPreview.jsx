  ...
  return (
    <>
      <div className="flexbox" style={{position: "relative", width: 650, height: 650}}>
        ...
        { art_file ? <CurrentImage currentImage={currentImage} setCurrentImage={setCurrentImage} light={light} /> : null}
        ...
        <div style={{ position: "absolute", top: 0, right: 0, zIndex: 20}}>
          <button onClick={front ? () => setCurrentImage({...currentImage, placement: "back"}) : () => setCurrentImage({...currentImage, placement: "front"})} className="orange-button full-width" style={{ width: 110, fontSize: 20, margin: 10, padding: "4px 10px", borderRadius: 10 }}>{currentImage.placement.toUpperCase()}</button>
        </div>
        {
          art_file ?
